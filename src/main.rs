...
fn generate_random_id(except_ids: &[u32]) -> u32 {
    ...
    loop {
        let res: u32 = rng.gen_range(1..3299);
...
fn deal_solving(id: &u32) {
    ...
    let mut lib_file = fs::OpenOptions::new()
        ...
        .unwrap();
    writeln!(lib_file, "mod {};", solution_name).expect("Failed to write mod file");
}
...
fn deal_problem(problem: &Problem, code: &CodeDefinition, write_mod_file: bool) {
    ...
    if write_mod_file {
        let mut lib_file = fs::OpenOptions::new()
            ...
            .open("./src/problem/mod.rs")
            .unwrap();
        writeln!(lib_file, "mod {};", file_name).expect("Failed to write mod file");
    }
}
